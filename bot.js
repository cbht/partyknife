...
function log(message) {
	...
	var now = new Date();
	console.log(`[partyknife Bot] [${now.getUTCFullYear()}-${toTwoDigitString(now.getUTCMonth()+1)}-${toTwoDigitString(now.getUTCDate())} ${toTwoDigitString(now.getUTCHours())}:${toTwoDigitString(now.getUTCMinutes())}:${toTwoDigitString(now.getUTCSeconds())}] ${message}`);
};
...
log(`Starting partyknife Bot version ${botVersion}`);
...
client.on("ready", () => {
	...
	client.user.setActivity("Super Smash Bros.", {"type": "PLAYING"}).catch(log);
});
...
function generateGame(gameWidth, gameHeight, numMines, message, isRaw, startsNotUncovered) {
	...
	if (isNaN(numMines)) {
		...
		if (numMines <= 0) {
			return "You think you can look clever by solving a partyknife game without mines? Not gonna happen my friend.";
		}
